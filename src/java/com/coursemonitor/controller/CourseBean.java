...
public class CourseBean implements Serializable {
    ...
    public String saveCourse() throws IOException {
        ...
        Course c = new Course(cid, cname, academicSession, getFilename(image), description,null,null);
        stud.save(c);
   ...
   public String assignCourse() {
       ...
       AssignCourse as = new AssignCourse(new AssignCourseId(course, year), cd.getCourseById(course),sd.getStaffByName(assignedCm), sd.getStaffByName(assignedCl),80,0);
       stud.save(as);
      return "index.xhtml?faces-redirect";
   }

    public String redirectReport(AssignCourseId id) {
        CourseDao cd=  new CourseDao();
        FacesContext context = FacesContext.getCurrentInstance();
        context.getExternalContext().getSessionMap().put("assigncourse",id);
        return "writeReport.xhtml?faces-redirect";
    }
    
      public String viewReport(AssignCourse ac) {
        FacesContext context = FacesContext.getCurrentInstance();
        context.getExternalContext().getSessionMap().put("assign",ac);
        return "viewReport.xhtml?faces-redirect";
    }
    ...
    public List<AssignCourse> getCourseByCourseLeader() {
         FacesContext context = FacesContext.getCurrentInstance();
    Map<String, String> requestMap = context.getExternalContext().getRequestParameterMap();
    String code = "";
    if (requestMap.get("filter")!=null)
     code = (String) requestMap.get("filter");

      if (code.equals("") || code.equals("all") || code.equals("write")) 
      {
     ...
     public List<AssignCourse> getCourseByCourseModerator() {
         FacesContext context = FacesContext.getCurrentInstance();
    Map<String, String> requestMap = context.getExternalContext().getRequestParameterMap();
    String code = "";
    if (requestMap.get("filter")!=null)
     code = (String) requestMap.get("filter");
      ...
      if (code.equals("") || code.equals("all") || code.equals("view")) 
      {
        AssignCourseDao cd = new AssignCourseDao();
        HttpServletRequest req = (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest();
